 ...
 * This queue orders elements FIFO (first-in-first-out). The <em>head</em> of the queue is
 * that element that has been on the queue the longest time. The <em>tail</em> of the
 * queue is that element that has been on the queue the shortest time. New elements are
 * inserted at the tail of the queue, and the queue retrieval operations obtain elements
 * at the head of the queue. Linked queues typically have higher throughput than
 ...
 * queue expansion. The capacity, if unspecified, is equal to {@link Integer#MAX_VALUE}.
...
public class ResizableCapacityLinkedBlockIngQueue<E> extends AbstractQueue<E>
	...
	/*
	 * A variant of the "two lock queue" algorithm. The putLock gates entry to put (and
	 * offer), and has an associated condition for waiting puts. Similarly for the
	 * takeLock. The "count" field that they both rely on is maintained as an atomic to
	 * avoid needing to get both locks in most cases. Also, to minimize need for puts to
	 * get takeLock and vice-versa, cascading notifies are used. When a put notices that
	 * it has enabled at least one take, it signals taker. That taker in turn signals
	 * others if more items have been entered since the signal. And symmetrically for
	 * takes signalling puts. Operations such as remove(Object) and iterators acquire both
	 * locks.
	 *
	 * Visibility between writers and readers is provided as follows:
	 *
	 * Whenever an element is enqueued, the putLock is acquired and count updated. A
	 * subsequent reader guarantees visibility to the enqueued Node by either acquiring
	 * the putLock (via fullyLock) or by acquiring the takeLock, and then reading n =
	 * count.get(); this gives visibility to the first n items.
	 *
	...
	/**
	 * Creates a {@code ResizableCapacityLinkedBlockIngQueue} with the given (fixed)
	 * capacity.
	 *
	 ...
	 * equal to the initial capacity of this queue less the current {@code size} of this
	 * queue.
	 *
	 * <p>
	...
	public void put(E e) throws InterruptedException {
		...
		try {
			 ...
			 * puts are shut out by lock), and we (or some other waiting put) are
			 * signalled if it ever changes from capacity. Similarly for all other uses of
			 * count in other wait guards.
			 */
	...
	/**
	 * Inserts the specified element at the tail of this queue, waiting if necessary up to
	 * the specified wait time for space to become available.
	 *
	 * @return {@code true} if successful, or {@code false} if the specified waiting time
	 ...
	 * immediately without exceeding the queue's capacity, returning {@code true} upon
	 * success and {@code false} if this queue is full. When using a capacity-restricted
	 * queue, this method is generally preferable to method {@link BlockingQueue#add add},
	...
	public boolean remove(Object o) {
		...
		fullyLock();
		try {
			for (Node<E> trail = head,
				 p = trail.next; p != null; trail = p, p = p.next) {
				if (o.equals(p.item)) {
					unlink(p, trail);
					return true;
				}
	...
	/**
	 * Returns {@code true} if this queue contains the specified element. More formally,
	 * returns {@code true} if and only if this queue contains at least one element
	 ...
	 * this queue. (In other words, this method must allocate a new array). The caller is
	...
	/**
	 * Returns an array containing all of the elements in this queue, in proper sequence;
	 * the runtime type of the returned array is that of the specified array. If the queue
	 ...
	 *
	 * <p>
	 * If this queue fits in the specified array with room to spare (i.e., the array has
	 * more elements than this queue), the element in the array immediately following the
	 ...
	 *
	 * <p>
	 * Suppose {@code x} is a queue known to contain only strings. The following code can
	 * be used to dump the queue into a newly allocated array of {@code String}:
	 *
	 ...
	 * {@code toArray()}.
	 *
	 * @param a the array into which the elements of the queue are to be stored, if it is
	...
	private class Itr implements Iterator<E> {
		 ...
		 * so that if hasNext() reports true, we will still have it to return even if lost
		 * race with a take etc.
		 */
	...
	static final class LbqSpliterator<E> implements Spliterator<E> {
		...
		private static <E> boolean judge(boolean exhausted, Node<E> h,
			...
			return !exhausted && ((h = current) != null || (h = q.head.next) != null) && h.next != null;
		}
		...
		public Spliterator<E> trySplit() {
			...
			if (judge(exhausted, h, q, current)) {
