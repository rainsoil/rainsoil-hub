...
public class XssUtil {
	 ...
	 * @author michael
	 */
	public class HtmlWhitelist extends org.jsoup.safety.Safelist {
		...
		protected boolean isSafeAttribute(String tagName, Element el, Attribute attr) {
			...
			if (HttpConstants.SRC.equalsIgnoreCase(attr.getKey())
					|| HttpConstants.HREF.equalsIgnoreCase(attr.getKey())) {
				String value = attr.getValue();
				if (StringUtils.hasText(value) && value.toLowerCase().startsWith(HttpConstants.JAVASCRIPT)) {
					return false;
				}
			}
			// 允许 base64 的图片内容
			if (HttpConstants.IMG.equals(tagName) && HttpConstants.SRC.equals(attr.getKey())
					&& attr.getValue().startsWith(HttpConstants.BASE64)) {
				return true;
			}
