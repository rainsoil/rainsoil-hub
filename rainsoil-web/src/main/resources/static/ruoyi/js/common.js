...
function setCookie(name, value, expireSecond) {
    ...
    date.setTime(date.getTime() +  expireSecond * 1000); //格式化为cookie识别的时间
    document.cookie = escape(name) + "=" + escape(value) + ";expires=" + date.toGMTString(); //将name设置为expireSecond 秒后过期,超过这个时间name这条cookie会消失
}